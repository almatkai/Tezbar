...
export async function initTauriBridge(): Promise<void> {
  ...
  const callBackend = (channel: string, ...args: unknown[]): Promise<any> => {
    return invoke('call_backend', { channel, payload: args })
  }
  ...
  const tezbar: TauriRaymesApi = {
    ...
    getExtensionInstallError: (extensionId: string) => callBackend('extensions:install-error', extensionId),
    ...
    extensionSearchTextChanged: (payload: any) => callBackend('extension:search-text-changed', payload),
    ...
    executeSearchAction: (action: any, context?: any) => callBackend('search:execute', { action, context }),
    ...
    setWindowContentHeight: (height: number, zoomFactor: number) => invoke('window_set_content_height', { height, zoomFactor }),
    ...
    updateQuickNote: (createdAt: number, text: string) => callBackend('notes:update', { createdAt, text }),
    deleteQuickNote: (createdAt: number) => callBackend('notes:delete', createdAt),
    
    terminalCreate: (request: any) => callBackend('terminal:create', request),
    terminalWrite: (sessionId: string, data: string) => callBackend('terminal:write', { sessionId, data }),
    terminalResize: (sessionId: string, cols: number, rows: number) => callBackend('terminal:resize', { sessionId, cols, rows }),
    terminalKill: (sessionId: string) => callBackend('terminal:kill', { sessionId }),
    ...
    agentRun: (task: string) => callBackend('agent:run', task),
    ...
    onStreamToken: (listener: (token: string) => void) => setupEventListener('stream-token', listener),
    onStreamDone: (listener: () => void) => setupEventListener('stream-done', listener),
    onStreamError: (listener: (msg: string) => void) => setupEventListener('stream-error', listener),
    onWindowShown: (listener: (payload: { resetUi: boolean }) => void) => setupEventListener('window-shown', listener),
    onWindowSnapGuides: (listener: (payload: { visible: boolean; active: boolean }) => void) => setupEventListener('window:snap-guides', listener),
    onVoiceHotkeyHold: (listener: (payload: { phase: 'press' | 'release' }) => void) => setupEventListener('voice:hotkey-hold', listener),
    onTerminalData: (listener: (event: any) => void) => setupEventListener('terminal:data', listener),
    onTerminalExit: (listener: (event: any) => void) => setupEventListener('terminal:exit', listener),
    onQuickNoteSaveShortcut: (listener: () => void) => setupEventListener('notes:quick-save-shortcut', listener),
    onAppSurfaceOpen: (listener: (surface: 'command' | 'settings' | 'clipboard') => void) => setupEventListener('app:open-surface', listener),
    onAgentEvent: (listener: (event: any) => void) => setupEventListener('agent:event', listener),
    onExtensionInstallProgress: (listener: (payload: any) => void) => setupEventListener('extension:install-progress', listener),
  ...
  void tezbar.getLlmConfig().then((config) => {
